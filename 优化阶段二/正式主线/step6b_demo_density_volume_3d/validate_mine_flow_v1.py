...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate and manifest the formal mine-scale multiscale v1 flow.")
...
def main() -> int:
    ...
    output_root = formal / "output" / version
    summary_paths = {
        ...
        "step7a": output_root / "step7a_small_scale/small_dfn_summary.json",
        "step7b": output_root / "step7b_medium_scale/medium_dfn_summary.json",
        "step7c": output_root / "step7c_large_fault/large_fault_dfn_summary.json",
        "step7d": output_root / "step7d_fused/fused_multiscale_summary.json",
        "step8": output_root / "step8_well_correction/well_corrected_dfn_summary.json",
        "step9": output_root / "step9_sections/cheye1_dfn_multibackground_sections/section_summary.json",
        "single_scale_export": output_root / "step8_well_correction/single_scale_dfn/single_scale_dfn_export_summary.json",
        "single_scale_diag": output_root / "step9_sections/single_scale_diagnostic_sections/single_scale_diagnostic_summary.json",
    }
    summaries = {name: read_json(path) for name, path in summary_paths.items()}
    step7c_dir = output_root / "step7c_large_fault"
    ...
    step7d_unified_vtk = output_root / "step7d_fused/fused_multiscale_dfn_raw_time.vtk"
    step8_unified_vtk = output_root / "step8_well_correction/well_corrected_dfn_raw_time.vtk"
    ...
    diag_rows = summaries["single_scale_diag"].get("images", [])
    diag_scales = {str(row["scale"]) for row in diag_rows}
    diag_scopes = {str(row["scope"]) for row in diag_rows}
    checks: dict[str, bool] = {
        ...
        "single_scale_export_has_three_scales": {"small", "medium", "large"}
        <= set(summaries["single_scale_export"]["products"].keys()),
        "single_scale_export_counts_closed": bool(
            summaries["single_scale_export"].get("checks", {}).get("scale_counts_sum_to_total")
        ),
        "single_scale_export_large_keeps_original_faults": bool(
            summaries["single_scale_export"].get("products", {}).get("large", {}).get("original_fault_group_included")
            and summaries["single_scale_export"].get("original_fault_cell_count", 0) > 0
        ),
        "single_scale_diag_has_three_scales": {"small", "medium", "large"} <= diag_scales,
        "single_scale_diag_scopes_match_config": diag_scopes
        <= {"overview", "local_200m"}
        and bool(diag_scopes),
        "single_scale_diag_images_nonempty": all(
            int(row.get("overlay", {}).get("dfn_segment_count", 0)) > 0
    ...
    vtk_paths = [
        output_root / "step7a_small_scale/small_dfn_raw_time.vtk",
        output_root / "step7b_medium_scale/medium_dfn_raw_time.vtk",
        original_surface_vtk,
        output_root / "step7c_large_fault/large_inferred_fault_surfaces_raw_time.vtk",
        step7c_unified_vtk,
        step7d_unified_vtk,
        step8_unified_vtk,
        output_root / "step8_well_correction/single_scale_dfn/small_scale_dfn_raw_time.vtk",
        output_root / "step8_well_correction/single_scale_dfn/medium_scale_dfn_raw_time.vtk",
        output_root / "step8_well_correction/single_scale_dfn/large_scale_dfn_raw_time.vtk",
    ]
    ...
    if checks["step7c_original_transport_files_exist"]:
        ...
        for name in (
            "step7c_original_surface_file_hash_preserved",
            "step7c_manifest_row_count_matches_summary",
            "unified_vtks_have_both_geometry_groups",
            "original_fault_geometry_fingerprint_continuous_step7c_to_step8",
            "original_fault_render_area_policy_is_predicted_median",
        ):
            checks[name] = False
    image_dir = output_root / "step9_sections/cheye1_dfn_multibackground_sections"
    ...
    manifest = {
        ...
        "single_scale_diag_images": [str(row["file"]) for row in diag_rows],
    }
    output_path = step6 / "flow_acceptance_mine_v1.json"
