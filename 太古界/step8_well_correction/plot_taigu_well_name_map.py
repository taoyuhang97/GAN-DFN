...
用法（在服务器上）：
  python3 plot_taigu_well_name_map.py --csv <annotations.csv> --vtk <trajectories.vtk> \
...
def plot_well_map(rows, polylines, out_path, title, transparent=False, bare=False, extent=None):
...
def main():
    ...
    parser.add_argument("--title", default="太古界井位与井轨迹平面图")
    ...
    extent = limits if all(value is not None for value in limits) else None
    plot_well_map(rows, polylines, args.out, args.title,
                  transparent=args.transparent, bare=args.bare, extent=extent)
    return 0
