...
> **太古界实际入口**：`train_and_predict_taigu_gr_rd_rs.py`（配置
> `configs/taigu_step4_gr_rd_rs_v1.json`，输出
> `output/taigu_step4_gr_rd_rs_v1`）。它训练两个地层专家
> （`上部复合层_RD_RS`、`太古界风化壳_RD_RS`）并对 Step2 v3 全部 17 口井预测，
> 输出合并密度曲线（唯一键 `WellName+TVD`）与裂缝点。设计记录见
> `taigu_step4_design_20260812.md`。下方正文是砂砾岩六专家 Step4 的说明，
> 不能直接用于太古界。
