...
# Repoint the wrapper to the formal stage1 core and the rebuilt 0.2 ms sample/result roots.
RUN_LSTM_SCRIPT = ROOT.parent / "run_lstm_experiment.py"
DEFAULT_DATA_DIR = Path(
    r"E:\项目\石油项目\断缝储\原始数据\wx数据\砂砾岩\优化阶段一\研究内容一\成像测井\裂缝样本_uniform_0p2ms"
)
DEFAULT_SAVE_ROOT = FLOW_RESULT_ROOT / "stage1_lstm_uniform_0p2ms"


def compute_valid_sequence_count(sub_df: pd.DataFrame, seq_len: int) -> int:
    if len(sub_df) < seq_len:
        return 0
    if "ROW_IN_WELL" not in sub_df.columns:
        raise ValueError("ROW_IN_WELL missing in filtered strata dataset")
    if "TIME" not in sub_df.columns:
        raise ValueError("TIME missing in filtered strata dataset")

    row_in_well = pd.to_numeric(sub_df["ROW_IN_WELL"], errors="coerce").to_numpy(dtype=np.float64)
    time_values = pd.to_numeric(sub_df["TIME"], errors="coerce").to_numpy(dtype=np.float64)
    half = seq_len // 2
    count = 0
    for idx in range(half, len(sub_df) - half):
        row_window = row_in_well[idx - half: idx + half + 1]
        time_window = time_values[idx - half: idx + half + 1]
        if not (np.all(np.isfinite(row_window)) and np.all(np.isfinite(time_window))):
            continue
        if not np.all(np.diff(row_window) == 1):
            continue
        if np.any(np.abs(np.diff(time_window) - EXPECTED_TIME_STEP) > TIME_STEP_TOLERANCE):
            continue
        count += 1
    return count
...
def export_filtered_strata_dataset(
    ...
    def _summary_builder(well_name: str, target_strata: str, sub: pd.DataFrame) -> dict:
        ...
        sequence_count = 0
        if not sub.empty:
            sequence_count = compute_valid_sequence_count(sub, seq_len)
