...
SAVE_DIR = r"/data/shared/project-oil/wx数据/砂砾岩/优化阶段一/研究内容一/裂缝存在性预测/LSTM/单井验证/成像测井裂缝预测/cnn+lstm"
...
def build_sequences_by_well(df, X_seis, X_log, y_cls, y_den, seq_len):
    X_seis_seq, X_log_seq, y_cls_seq, y_den_seq = [], [], [], []
...
# ================= 中心段过采样 =================
def oversample_center_segments(seis, log, y_cls, y_den,
                               target_ratio=0.35,
                               edge_exclude=1,
                               random_state=42):
    """
    ...
    return seis_new, log_new, y_cls_new, y_den_new
...
def cnn_lstm_permutation_importance(model, X_seis, X_log, y, loss_fn, device="cpu"):
    ...
    for f in range(n_features):
        ...
        for _ in range(5):
            ...
            with torch.no_grad():
                cls_out, _ = model(X_seis, X_log_perm)
...
for val_well in well_names:
    ...
    X_val_all = scaler.transform(val_df[features].values)
    ...
    # ---------- 构造序列 ----------
    X_train_seis_seq, X_train_log_seq, y_train_seq, y_train_den_seq = build_sequences_by_well(
        train_df,
        X_train_seis,
        X_train_log,
        y_train,
    ...
    X_val_seis_seq, X_val_log_seq, y_val_seq, y_val_den_seq = build_sequences_by_well(
        val_df,
        X_val_seis,
        X_val_log,
        y_val,
    ...
    for epoch in range(EPOCHS):
        ...
        for xb_seis, xb_log, yb_cls, yb_den in pbar:
            optimizer.zero_grad()
            cls_out, den_out = model(xb_seis, xb_log)
        ...
        model.eval()
        with torch.no_grad():
            cls_out_val, den_out_val = model(X_val_seis_t, X_val_log_t)
    ...
    model.eval()
    with torch.no_grad():
        cls_out, den_out = model(X_val_seis_t, X_val_log_t)
