...
def build_fault_surface_patch_df(panel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, panel_row in panel_df.iterrows():
        ...
        panel_length = max(float(panel_row.get("PanelLength", 20.0)), 20.0)
        panel_height = max(float(panel_row.get("PanelHeight", 8.0)), 8.0)
        strike_tile_count = max(1, int(np.ceil(panel_length / 55.0)))
        dip_tile_count = max(1, int(np.ceil(panel_height / 16.0)))
        strike_edges = np.linspace(-0.5 * panel_length, 0.5 * panel_length, strike_tile_count + 1)
        dip_edges = np.linspace(-0.5 * panel_height, 0.5 * panel_height, dip_tile_count + 1)
        tile_counter = 0
        for strike_idx in range(strike_tile_count):
            s0 = float(strike_edges[strike_idx])
            s1 = float(strike_edges[strike_idx + 1])
            for dip_idx in range(dip_tile_count):
                d0 = float(dip_edges[dip_idx])
                d1 = float(dip_edges[dip_idx + 1])
                tile_counter += 1
                local_center = center + 0.5 * (s0 + s1) * strike_vec + 0.5 * (d0 + d1) * dip_vec
                tile_length = max(4.0, 0.90 * abs(s1 - s0))
                tile_height = max(2.0, 0.90 * abs(d1 - d0))
                surface_rows.append(
                    make_patch_row_from_axes(
                        center=local_center,
                        u_vec=strike_vec,
                        ...
                        extra={
                            "Confidence": 0.98,
                            "PatchOriginCode": PATCH_ORIGIN_TEXT_TO_CODE["fault_surface"],
                            "PatchOriginText": "fault_surface",
                            "FaultActionCode": FAULT_ACTION_TEXT_TO_CODE["surface"],
                            "FaultActionText": "surface",
                            "FaultDistanceMs": 0.0,
                            ...
                            "ConnectionType": "original",
                            "AggregationMode": "original",
                            "ScaleClass": "macro_core",
                            "CorridorSupport": 1,
                            "IsSupplemented": 0,
                            "FractureSet": -1,
                            "FaultSurfaceTileIndex": int(tile_counter),
                        },
                    )
                )
...
def finalize_output_dataframe(
    ...
    frames = [frame for frame in [original_df, parallel_df, perpendicular_df, surface_df] if frame is not None and not frame.empty]
    if not frames:
        return pd.DataFrame()
...
def run_fault_postfusion(
    ...
    surface_df = build_fault_surface_patch_df(panel_df)
    final_df = finalize_output_dataframe(kept_df, parallel_df, perpendicular_df, surface_df)

    output_csv = run_dir / "regional_dfn_fault_embedded.csv"
    output_vtk = run_dir / "regional_dfn_fault_embedded_raw.vtk"
    write_csv_utf8(final_df, output_csv)
    write_df_to_regional_vtk(final_df, f"{title}_fault_embedded", output_vtk, scalar_types)

    parallel_csv = run_dir / "fault_parallel_patches.csv"
    perpendicular_csv = run_dir / "fault_perpendicular_patches.csv"
    surface_csv = run_dir / "fault_surface_patches.csv"
    parallel_vtk = run_dir / "fault_parallel_patches_raw.vtk"
    perpendicular_vtk = run_dir / "fault_perpendicular_patches_raw.vtk"
    surface_vtk = run_dir / "fault_surface_patches_raw.vtk"
    ...
    if not surface_df.empty:
        write_csv_utf8(surface_df, surface_csv)
        write_df_to_regional_vtk(surface_df, f"{title}_fault_surface", surface_vtk, scalar_types)
    ...
    summary = {
        ...
        "fault_surface_patch_count": int(len(surface_df)),
        "final_patch_count": int(len(final_df)),
        ...
        "fault_surface_csv": str(surface_csv) if not surface_df.empty else "",
        "fault_surface_vtk": str(surface_vtk) if not surface_df.empty else "",
...
def main() -> None:
    ...
    append_lines_to_docx(
        ...
        lines=[
            ...
            f"final_patch_count: {summary['final_patch_count']}",
    ...
    print(f"final_patch_count: {summary['final_patch_count']}")
