...
def visualize_npy_fractures(file_path):
    ...
    # 常用视角
    # pl.view_xy()  # 从 Z 轴正方向看 (俯视)
    # pl.view_yz()  # 从 X 轴正方向看
    # pl.view_xz()  # 从 Y 轴正方向看
    pl.show()

# def visualize_npy_fractures(file_path):
#     """可视化裂缝片（支持单个或多个），同时显示正视图和左视图"""
#     data = np.load(file_path, allow_pickle=True)
#
#     # 支持列表或字典
#     if isinstance(data, np.ndarray) and data.dtype == object:
#         data = data.tolist()
#
#     pl = pv.Plotter(shape=(1, 2), window_size=(1200, 600))  # 左右两视图
#
#     # 构建 mesh 列表
#     meshes = []
#     if isinstance(data, list):
...
#     for mesh in meshes:
#         pl.add_mesh(mesh, color='red', show_edges=True, opacity=0.6)
#     pl.add_text(f"正视图 (X 轴向前)", position='upper_left')
#     pl.view_vector([1, 0, 0])  # 相机方向沿 X 正
#     pl.camera.zoom(1.2)
#
#     # 右侧子图: 侧视图 (Y 轴向前)
#     pl.subplot(0, 1)
#     for mesh in meshes:
#         pl.add_mesh(mesh, color='red', show_edges=True, opacity=0.6)
#     pl.add_text(f"侧视图 (Y 轴向前)", position='upper_left')
#     pl.view_vector([0, 1, 0])  # 相机方向沿 Y 正
#     pl.camera.zoom(1.2)
#
#     pl.show()
