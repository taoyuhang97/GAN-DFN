...
# ========= 1) 读取“FaultStick-Landmark5000.dat”文件 =========
    ...
    with open(txt_path, "r", encoding="utf-8") as f:
        for ln in f:
            ...
            try:
                ...
                # 解析其它字段
                sign = parts[3]  # 保留为字符串，不进行转换
                fault = parts[7]  # 断层名称，这里是“F01”样式
...
def group_by_fault(arr):
    ...
    for (x, y, t, sign, fault, inline, xline) in arr:
        ...
        groups[fault]["sign"].append(sign)  # 保留 sign 字段为字符串
    ...
    for f in groups:
        ...
        groups[f]["sign"]   = np.asarray(groups[f]["sign"], dtype=object)  # 保持 sign 为字符串
...
def build_fault_surfaces(txt_path):
    ...
    Sign = data[:, 3].astype(str)  # Sign 字段现在是字符串类型
